import checkForNewPosts from './updater.js';
...
const runApp = () => {
  ...
  // Функция запуска проверки новых постов
  const startCheckingFeeds = () => {
    if (!state.isChecking) {
      state.isChecking = true;
      checkForNewPosts(state, fetchRss, parseRss); // Запускаем проверку
    }
  };
  ...
  // Обертка состояния с наблюдением
  ...
  elements.form.addEventListener('submit', (event) => {
    ...
    validateUrl(url, state.feeds.map((feed) => feed.url))
      ...
      .then((data) => {
        ...
        startCheckingFeeds(); // Запускаем проверку фидов, если еще не запущена
      })
      .catch((err) => {
